...
void AddNumber(Node **pp_head, Node **pp_tail, int data)
{
	...
	{// 노드 데이터가 없다...
		//*p_head 값이 NULL이라서 첫 노드가 추가됨
		*pp_head = (Node*)malloc(sizeof(Node));
		*pp_tail = *pp_head;
	}
	(*pp_tail)->number = data; // 숫자 저장함
	(*pp_tail)->p_next = NULL; // 마지막 노드여서
...
int main(void)
{
	...
	while (1)
	{
		...
		if (9999 == temp)break;
		// 노드의 시작과 끝을 기억하는 포인터의 주소 값과입력된 숫자를 전달
		AddNumber(&p_head, &p_tail, temp);
	}
	// ========== 2.모든 입력받은 숫자를 출력==========
	// 입력된 숫자를 출력하기 위해서 노드를 탐색할 
	// 포인터에 시작 노드의 주소 값을 대입
	p = p_head;
	while (NULL != p)
	{
		if (p != p_head)
	...
	p_tail = p_head;// 반복문을 나오면 p_head 값은 NULL, p_tail 도 NULL로 변경
